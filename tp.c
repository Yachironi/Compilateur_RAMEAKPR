...
int main(int argc, char **argv) {
   ...
   * c'est possible.
   */
  printf("Avant yyparse\n");  
  res = yyparse();
  printf("res=%d\n", res);
  printf("On essaye d'afficher l'arbre a partir de l'axiome (Programme) qui est dans tp.y\n");
  if (programme == NULL) {
    printf("Programme est NULL\n");
  }
  else{
    printf("Programme n'est pas NULL\n");
...
PCLASS empilerClasse(PCLASS listeClasse, PCLASS classe){
  if(listeClasse==NULL){
    listeClasse=classActuel;
  }
  else{
    PCLASS tmp=listeClasse;
    while(tmp->suivant!=NULL){
      tmp=tmp->suivant; 
    }
    tmp->suivant=classActuel;
  }
  return listeClasse;
}
...
PCLASS makeClasse(char *nom,PVAR param_constructeur,TreeP corps_constructeur,PMETH liste_methodes,PVAR liste_champs, PCLASS classe_mere, int isExtend){
  ...
  if(listeDeClass==NULL){
      ...
      while(tmp->suivant!=NULL){
        ...
        printf("MAGIE : je parcours le suivant %s\n",tmp->nom );

        tmp=tmp->suivant; 
      }
      tmp->suivant=res;
    }
  /* avant il y avait la mise a jour de listeClass 
  printf("++++++++++++++++++++++++++++++++++++++\n");
  printf("++++ dans makeClasse : listeDeClass=\n");
  printClasse(listeClass);
  printf("++++++++++++++++++++++++++++++++++++++\n");
  */
